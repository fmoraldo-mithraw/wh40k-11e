...
| `increment` / `set` / `decrement` / `floor` sur `<constraint id>` | 1 923 / 1 094 / 65 / 2 | 🟡 | Bornes dynamiques : max d'un modèle (`modelMaxScaling` + `modEffectiveValue`), bornes de groupe de composition (`buildDynBounds`), plafond d'un choix (`choiceUnitMax` : `set` lessThan, `increment` atLeast, `repeats` par N modèles), **max d'un choix conditionné à la catégorie de l'unité** (`effEntryConstraint` + `unitCategoryIds`, même évaluateur que `entryHiddenFor` : les pools partagés T'au « Drones (0-2) » `set 1` sous ancêtre Fireknife/Starscythe/Sunforge/Stealth → une Crisis ne prend jamais deux fois le même drone ; test §8), redimensionnement d'un groupe importé (lien, inconditionnel). Non couvert : bornes de groupes d'options conditionnées à autre chose que la taille ou la catégorie (détachement, amélioration), `floor`. |
...
2. **Options masquées puis révélées** — porte runtime `{b, m, min}` (mécanisme générique, plus aucun cas réel : les upgrades Assassins Veiled Blade / C'tan Pantheon of Woe sont devenus des améliorations de détachement côté données), porteur révélé (Pennant of Remembrance) ; groupes Daemons morts, Crusade et lien Warlord Deathleaper hors champ.
